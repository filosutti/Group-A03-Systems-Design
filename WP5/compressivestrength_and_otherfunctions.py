...
def calculate_Ixx(y, design_params):
    ...
    # 1. Skin Contribution (Top & Bottom)
    # I = 2 * (Area * distance^2) -> ignoring negligible self-inertia about centroid
    A_skin_flange = b_box * t_skin
    I_skin = 2 * (A_skin_flange * (h_box / 2)**2)
    
    # 2. Spar Contribution (Webs)
    # I = 2 * (b * h^3 / 12)
    I_spar = 2 * (t_spar * h_box**3 / 12)
    
    # 3. Stringer Contribution
    # Area of one L-stringer (approximate as two rectangles)
    A_str = (w_str * t_str) + ((w_str - t_str) * t_str)
    # Total stringer area (Top + Bottom)
    A_str_total = 2 * n_str * A_str
    # Steiner term: A * d^2
    I_str = A_str_total * (h_box / 2)**2
